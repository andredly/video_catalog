...
describe('actions moviesReducer', () => {
    ...
    it('creates ADD_MOVIES_TO_STATE when fetching moves has been done', () => {
        ...
        const expectedActions = [
            {type: FETCH_MOVIES_PENDING},
            {
                type: ADD_MOVIES_TO_STATE, movies: {
                    data: ["testData"],
                    offset: 10,
                    total: 10
                }
        ...
        return store.dispatch(actions.loadMoreMovies({}))
            .then(() => {
                expect(store.getActions())
                    .toEqual(expectedActions);
            });
    ...
    it('creates GET_MOVIE_DETAILS when fetching moves has been done', () => {
        ...
        return store.dispatch(actions.loadMovieDetails(10))
            .then(() => {
                expect(store.getActions())
                    .toEqual(expectedActions);
            });
